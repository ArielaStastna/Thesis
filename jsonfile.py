...
class Process:
    ...
    def _process_nested_keys(data, keys, anonymization_function, elasticsearch):
            ...
            if keys[0] in data:
                ...
                if keys[0] in elasticsearch.IP_KEYS:
                    ...
                    if isinstance(data[keys[0]], list):
                        data[keys[0]] = [Process._handle_ip_addresses(item, elasticsearch) for item in data[keys[0]]]
                    # If the value is not a list, apply the handle_ip_addresses function to the value
                    else:
                        data[keys[0]] = Process._handle_ip_addresses(data[keys[0]], elasticsearch)
        ...
        # If there are still multiple keys remaining in the keys list
            ...
            if key in data:
                ...
                Process._process_nested_keys(data[key], keys[1:], anonymization_function, elasticsearch)
    ...
    def _anonymize_keys(data, key_anonymization_mapping, elasticsearch):
        if isinstance(data, dict):
            ...
            for key, anonymization_function in key_anonymization_mapping.items():
                ...
                Process._process_nested_keys(data, keys, anonymization_function, elasticsearch)
            # Recursively call the anonymize_keys function on each value in the dictionary that is a dictionary or list
            for value in data.values():
                if isinstance(value, (dict, list)):
                    Process._anonymize_keys(value, key_anonymization_mapping, elasticsearch)
